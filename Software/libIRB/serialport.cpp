...
void SerialPort::sendData(std::span<const uint8_t> data) const
{
#if defined(__linux__)
    const int count = ::write(d->fd, data.data(), data.size());
...
auto SerialPort::readAllData() const -> std::vector<uint8_t>
{
    ...
    {
#if defined(__linux__)
        const int count = ::read(d->fd, data.data(), data.size());
        const bool success = count >= 0 && static_cast<size_t>(count) == data.size();
#elif defined(_WIN32)
        ::DWORD count;
